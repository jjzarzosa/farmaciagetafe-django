...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_email.send()
            #messages.success(request, 'Gracias por registrate. Hemos enviado un email de verificación a tu dirección de correo. Por favor verifica esto.')
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Enhorabuena. Tu cuenta ha sido dada de alta.')
        return redirect('login')
    else:
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            # Reset password
            current_site = get_current_site(request)
            mail_subject = 'Por favor, introduce un nuevo password'
            ...
            send_email = EmailMessage(mail_subject, message, to=[to_email])
            send_email.send()
            messages.success(request, 'Email del reseteo de la contraseña se ha enviado a tu cuenta de correo.')
            return redirect('login')
        else:
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Por Favor reinicia tu contraseña.')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request, 'La contraseña se ha reiniciado correctamente.')
            return redirect('login')
        else:
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.error(request, 'Por favor, introduce tu contraseña actual.')
                return redirect('change_password')
        else:
            messages.error(request, 'Las contraseñas no coinciden!')
            return redirect('change_password')
